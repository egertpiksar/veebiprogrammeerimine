<?php
	...
	function signIn($email, $password){
		...
		$stmt = $mysqli->prepare("SELECT id, firstname, lastname, email, password FROM vp_piksar WHERE email = ?");
	...
	function readAllIdeas(){
		$ideas = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("SELECT id, idea, color FROM vp_userideas WHERE userid = ?");
		...
		//$result = array();
		while ($stmt->fetch()){
			$ideas .= '<p style="background-color: ' .$color .'">' .$idea .' | <a href="edituseridea.php?id=' .$ideaId .'">Toimeta</a>' ."</p> \n";
			//lisame lingi: | <a href="edituseridea.php?id=6"> Toimeta
		}
		$stmt->close();
		$mysqli->close();
		return $ideas;
